...
def process_predict_blackhole_combine(combine_model_file_path, max_ability, q_input, q_output):
    ...
    combine_model = tf.keras.models.load_model(combine_model_file_path)
    while True:
        em = q_input.get(True)
        if em is None:
            break
        ...
        x = extract_combine_data(x, 1)

        combine_predict_y = combine_model.predict(x)
        isB_predict = combine_predict_y[0, 1] > 0.5
...
ProcessCtl_dict_combine = dict()
ProcessCtl_dict_combine["running_label"] = False
ProcessCtl_dict_combine["key"] = 0
DectectandBan_combine_q_input = multiprocessing.Queue()
DectectandBan_combine_q_output = multiprocessing.Queue()

# 使用训练好的model 在消息投递时候 增加对对端节点的判定
# Scenario 要响应 genpkt swappkt事件 和 最后的结果查询事件
class DTNScenario_Prophet_Blackhole_DectectandBan_combine(object):
    ...
    def __init__(self, scenarioname, list_selfish, num_of_nodes, buffer_size, total_runningtime):
        ...
        self.combine_model_file_path = os.path.join(dir, 'combine_model.h5')

        self.MAX_Ability = (10000, 'Max Process Ability', 'Continue')
        global ProcessCtl_dict_combine
        global DectectandBan_combine_q_input
        global DectectandBan_combine_q_output
        if not ProcessCtl_dict_combine["running_label"]:
            ProcessCtl_dict_combine["running_label"] = True
            j = multiprocessing.Process(target=process_predict_blackhole_combine, args=(
                self.combine_model_file_path, self.MAX_Ability, DectectandBan_combine_q_input, DectectandBan_combine_q_output))
            j.daemon = True
            j.start()
    ...
    def print_res(self, listgenpkt):
        ...
        global ProcessCtl_dict_combine
        global DectectandBan_combine_q_input
        if ProcessCtl_dict_combine["running_label"] == True:
            DectectandBan_combine_q_input.put(None)
            ProcessCtl_dict_combine["running_label"] = False
    ...
    def __detect_blackhole(self, a_id, b_id, runningtime):
        ...
        global DectectandBan_combine_q_input
        global DectectandBan_combine_q_output
        request_element = (ProcessCtl_dict_combine["key"], new_x.copy(), i_isSelfish)
        ProcessCtl_dict_combine["key"] = ProcessCtl_dict_combine["key"] + 1
        DectectandBan_combine_q_input.put(request_element)

        result_element = DectectandBan_combine_q_output.get(True)
