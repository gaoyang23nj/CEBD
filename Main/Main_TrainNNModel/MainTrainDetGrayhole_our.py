...
def train_indirect(lines_train, lines_val, h5_indirect_filepath):
    ...
    # 参数设置
    log_dir = 'logs_gray_time_indirect\\'
    # update_freq == 'batch'
    logging = tf.keras.callbacks.TensorBoard(log_dir=log_dir)
    checkpoint = tf.keras.callbacks.ModelCheckpoint(log_dir + 'ep{epoch:03d}-loss{loss:.3f}-val_loss{val_loss:.3f}.h5',
        monitor='val_loss', save_weights_only=False, save_best_only=False, save_freq='epoch')
...
if __name__ == "__main__":
    ...
    eve_dirs = ["E:\\collect_data_grayhole_time"]
    ...
    anno_gk_filepath = os.path.join(annotation_dir, "anno_grayhole_RWP.txt")
    if os.path.exists(anno_gk_filepath):
        os.remove(anno_gk_filepath)
    print(eve_dirs, anno_gk_filepath)
    build_anno(eve_dirs, anno_gk_filepath)

    anno_filepath = anno_gk_filepath
    ...
    h5_direct_filepath = os.path.join(ml_dir, "our_direct_model_RWP.h5")
    h5_indirect_filepath = os.path.join(ml_dir, "our_indirect_model_RWP.h5")
