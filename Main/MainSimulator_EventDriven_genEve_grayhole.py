    ...
    def init_scenario_grayhole_todetect(self):
        ...
        for j in range(5):
            ...
            for dropratio_i in tmplist:
                ...
                index += 1
                tmp_senario_name = 'scenario' + str(index)+'_grayhole_0_' + str(tmp)+'_ratio_0_'+str(dropratio_i)
                tmpscenario = DTNScenario_Prophet_Grayhole(tmp_senario_name, malicious_indices, dropratio, self.MAX_NODE_NUM, 20000)
                self.scenaDict.update({tmp_senario_name: tmpscenario})

                index += 1
                tmp_senario_name = 'traindata_' + str(self.ENCO_ID) + '_' + str(self.THR_PKT_GEN_CNT) + '_' + 'scenario' + str(index)+ '_grayhole_todetect_0_' + str(tmp)+'_ratio_0_'+str(dropratio_i)
