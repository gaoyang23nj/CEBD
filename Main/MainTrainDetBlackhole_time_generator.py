...
def tmp_train(lines_train, lines_val, h5_direct_filepath, h5_indirect_filepath):
    ...
    direct_model.fit_generator(mygenerator(lines_train, batch_size),
...
if __name__ == "__main__":
    ...
    eve_dir = "..\\Main\\collect_data_blackhole_time"
    ...
    if not os.path.exists(ml_dir):
        ...
        print(ml_dir + ' exist')
    h5_direct_filepath = os.path.join(ml_dir, "direct_model.h5")
    if os.path.exists(h5_direct_filepath):
        os.remove(h5_direct_filepath)
    h5_indirect_filepath = os.path.join(ml_dir, "indirect_model.h5")
    ...
    tmp_train(lines[: num_train], lines[num_train : num_train + num_val], h5_direct_filepath, h5_indirect_filepath)

    # test(lines[num_train + num_val:], h5_direct_filepath)
