...
def test(lines_test, h5_filepath):
    ...
    # y.shape 100*1;     # x.shape 100*301;
    y_final = np.zeros(shape=(100 * num_testlines, 1))
    x_final = np.zeros(shape=(100 * num_testlines, NUM_of_INPUTS))
    for i in range(num_testlines):
        y, x = process_data_npz(lines_test[i])
        # 对于0.npz 从0行到99行
        y_final[i * 100: (i + 1) * 100] = y
        x_final[i * 100: (i + 1) * 100] = x
...
if __name__ == "__main__":
    ...
    eve_dir = "..\\Main\\collect_data_blackhole_ex"
    ml_dir = "..\\Main\\ML_blackhole_ex"
    ...
    annotation_path = "..\\Main\\anno_blackhole_ex"
