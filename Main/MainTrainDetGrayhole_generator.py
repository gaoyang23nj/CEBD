...
def build_anno(eve_dir, annotation_path):
    # 把文件名 和 对应的数据源 洗出来
    npz_tunple_list = []
    scen_dirs = os.listdir(eve_dir)
    for scen_dir in scen_dirs:
        scen_path = os.path.join(eve_dir, scen_dir)
        npz_dirs = os.listdir(scen_path)
        for npz_dir in npz_dirs:
            npz_dir_path = os.path.join(scen_path, npz_dir)
...
if __name__ == "__main__":
    ...
    eve_dir = "..\\Main\\collect_data_grayhole"
