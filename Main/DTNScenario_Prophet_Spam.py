...
# (2) pkt一直保存在 src_id(spam) buffer里面，遇到合适情况(符合prophet要求 或者 逢人就转发(类似epidemic))就转发出去
...
class DTNScenario_Prophet_Spam(object):
    ...
    def __init__(self, scenarioname, list_selfish, num_of_nodes, buffer_size):
        ...
        for node_id in range(num_of_nodes):
            if node_id in list_selfish:
                tmpRouter = RoutingSpam(node_id, num_of_nodes)
            else:
    ...
    def swappkt(self, runningtime, a_id, b_id):
        ...
        if isinstance(self.listRouter[a_id], RoutingSpam) and isinstance(self.listRouter[b_id], RoutingSpam):
            # ================== 报文 交换; a_id是spam b_id是spam==========================
            self.sendpkt_fromspam(runningtime, a_id, b_id)
            self.sendpkt_fromspam(runningtime, b_id, a_id)
        elif isinstance(self.listRouter[a_id], RoutingSpam):
            # ================== 报文 交换; a_id是spam b_id是正常prophet==========================
            self.sendpkt_fromspam(runningtime, a_id, b_id)
            self.sendpkt(runningtime, b_id, a_id)
        elif isinstance(self.listRouter[b_id], RoutingSpam):
            # ================== 报文 交换; a_id是正常prophet b_id是spam==========================
            self.sendpkt(runningtime, a_id, b_id)
            self.sendpkt_fromspam(runningtime, b_id, a_id)
        elif (not isinstance(self.listRouter[a_id], RoutingSpam)) and (not isinstance(self.listRouter[b_id], RoutingSpam)):
            # ================== 报文 交换; a_id是正常prophet b_id是正常prophet==========================
            self.sendpkt(runningtime, a_id, b_id)
            self.sendpkt(runningtime, b_id, a_id)

    def sendpkt_fromspam(self, runningtime, a_id, b_id):
        P_b_any = self.listRouter[b_id].get_values_before_up(runningtime)
        P_a_any = self.listRouter[a_id].get_values_before_up(runningtime)
    ...
    def print_res_pure(self, listgenpkt):
        ...
        for tunple in listgenpkt:
            ...
            if (not isinstance(self.listRouter[src_id], RoutingSpam)) and (not isinstance(self.listRouter[dst_id], RoutingSpam)):
        ...
        total_pkt_hold = 0
        for i_id in range(len(self.listNodeBuffer)):
            if not isinstance(self.listRouter[i_id], RoutingSpam):
                list_succ = self.listNodeBuffer[i_id].getlistpkt_succ()
                tmp_succnum = 0
                for i_pkt in list_succ:
                    # 这样 src_id 和 dst_id 都是 正常prophet node
                    if not isinstance(self.listRouter[i_pkt.src_id], RoutingSpam):
                        tmp_delay = i_pkt.succ_time - i_pkt.gentime
                        total_delay = total_delay + tmp_delay
                        tmp_succnum = tmp_succnum + 1
                total_succnum = total_succnum + tmp_succnum
...
class RoutingSpam(RoutingProphet):
    def __init__(self, node_id, num_of_nodes):
        super(RoutingSpam, self).__init__(node_id, num_of_nodes)
